...
Web 3.0이란 단어는 너무 많이 들었지만 어디서부터 알아가야할 지 막막했다. 소프트웨어 개발자로 일하면서, 눈 앞의 지식을 취하는 것이 바빠 거시적인 흐름에는 크게 관심을 두지 않은 것이 조금은 부끄럽다. 먼저 그동안 막연히 여겼던 것부터 찾아봐야겠다고 생각했다.
...
2. Smart contract 개발에 도전해보기
    ...
    Smart contract는 Web 3.0 생태계의 핵심 요소이기 때문에, 어떻게 동작하는지 이해하거나 개발할 수 있으면 좋을 것이다. 블록체인 플랫폼에서 사용되는 `Solidity`, `Rust`, `Go` 등의 언어를 접하거나 `Remix`라는 IDE를 활용하여 접근해볼 수 있다.
